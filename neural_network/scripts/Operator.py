...
class Operator(IOperator):
	...
	def _post(self):
		...
		if self._state == States.Connected and self._detect_frame is not None and (datetime.now() - self._post_time).seconds > self._config['timeouts']['detection']:
	...
	def stream_frame(frame):
		"""Кодирует кадр в поток формата изображения png."""
		stream = BytesIO()
		stream.write(frame)
		stream.seek(0)
		return stream
